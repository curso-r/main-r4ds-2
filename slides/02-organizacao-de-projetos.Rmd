...
- Se todos os resultados parciais de uma análise estiverem disponíveis a qualquer momento, `r blue("diminui o incentivo para a escrita de código reprodutível")`.

- Se todo o histórico de comandos for acessível, `r blue("acaba a necessidade de experimentos controlados")`.

- Ao dependermos ativamente do `.Rdata`, `r blue("se acidentalmente sobrescrevemos um objeto")` relevante e o código para recriá-lo não estiver mais acessível, `r blue("não haverá nenhuma forma confiável de recuperá-lo")`.

- A menos que pretendamos sentar com colegas para explicar como utilizar os objetos do `.RData` e do `.Rhistory`, `r blue("não pode-se esperar que outra pessoa seja capaz de reproduzir uma análise")`.

- O R trata todos os objetos guardados na memória igualmente, sem levar em conta sua utilidade ou tamanho. Isso significa que ele também irá armazenar nos arquivos ocultos todas as bases de dados da sessão (não importando quão grande sejam). Assim, `r blue("o")` `.RData` `r blue("pode ser um arquivo de múltiplos gigabytes")`.

---
class: center, middle
# `r blue("Rproj e diretórios")`
...
`r blue("Felizmente o RStudio possui uma ferramenta incrível que auxilia na tarefa de consolidar todos os recursos necessários para uma análise.")` 
...
`r blue("O primeiro passo para sair dessa situação é transformar tudo em funções")`.
...
`r blue("Qual a melhor maneira de se ter controle sobre os pacotes utilizados na nossa análise?")`
...
- `r blue("Git é um software que você instala no computador.")`
...
- `r blue("GitHub é um site que você acessa na internet.")`
...
`r blue("Pacotes do R e repositórios do GitHub são melhores amigos.")`
