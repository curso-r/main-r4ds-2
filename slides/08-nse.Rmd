...
**tidyeval**, a avaliação não-padrão utilizada pelas funções do `{tidyverse}` e
outros pacotes feitos para trabalhar com o mesmo paradigma.

- O mundo sem tidyeval é extremamente verborrágico, pois a tabela precisa ser
...
- O operador que vai resolver nosso problema é o `{{` (lê-se _curly curly_), que
permite interpolar o código, ou seja, avaliá-lo antes da captura.
...
reticência já possui as capacidades de tidyeval:
...
- Quando o "lado esquerdo" (**antes de um igual**) de uma expressão com tidyeval
...
tidyeval, precisamos apenas usar o pronome `.data`:
...
- Para saber mais sobre NSE, tidyeval e metaprogramação, dê uma olhada nos
